...
- [~] **Bước 4 — Migrate 577 `<button>` thô sang `Button` (đang làm, 2026-07-05)**: tiến hành thủ công từng file theo yêu cầu chủ dự án ("làm dần từng file có kiểm tra, không dồn 1 lượt"), KHÔNG dùng codemod tự động hàng loạt (đã thử viết codemod AST, bị auto-mode classifier chặn đúng vì vi phạm tinh thần "không dồn 1 lượt" — đã dừng, quay lại sửa tay). **Tiến độ: đã xong 84/577 nút** (~40 file, ưu tiên file nhỏ 1-2 nút trước, bao gồm cả 2 file nền tảng rủi ro cao `components/shared/ui/Input.tsx` và `Dropdown.tsx` — đã spot-check bằng trình duyệt qua modal Cài đặt + Phân ca, không lỗi console/UI). Pattern chuẩn: `<Button variant="ghost" className="bg-transparent hover:bg-transparent border-0 rounded-none h-auto w-auto p-0 text-inherit <class gốc>">` — xem giải thích cơ chế merge class ở CHANGELOG "Bước 3". Còn lại ~493 nút, gồm cả các file lớn/phức tạp cố ý để sau: `EditShiftModal.tsx` (28, lưới lịch phức tạp), `CompetitionTab.tsx` (21), `PhanCaView.tsx` (17), `ControlPanel.tsx` (16). Verify sau mỗi batch: `eslint` (file vừa sửa), `tsc --noEmit` (toàn repo), định kỳ `npm run build` + `lint:ratchet` — tất cả đều sạch tại checkpoint này.
