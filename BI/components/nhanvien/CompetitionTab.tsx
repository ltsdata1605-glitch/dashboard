...
export const CompetitionTab: React.FC<CompetitionTabProps> = ({
    ...
    const cardTitle = (
        <div className="flex flex-col items-start leading-none py-1">
            <span className="js-report-title text-3xl font-extrabold uppercase text-primary-700">HIỆU QUẢ THI ĐUA NHÂN VIÊN ĐẾN NGÀY {getYesterdayDateString()}</span>
            <span className="text-sm italic text-slate-500 mt-2 font-medium">"Thi đua là động lực, hiệu quả là mục tiêu - Vượt qua giới hạn, khẳng định bản thân."</span>
        </div>
    );
                ...
                <div className="px-6 pb-6 pt-4">
                    <div className="border-b border-slate-200 dark:border-slate-700 pb-2">
                        <nav className="flex space-x-2 items-end flex-wrap -mb-px" aria-label="Tabs">
                            ...
                            <button onClick={() => { setActiveCompetitionTab('tong'); setActiveVersionName(null); }} className={`px-4 py-2 text-sm font-semibold rounded-t-md transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-400 ${activeVersionName === null && activeCompetitionTab === 'tong' ? 'border border-b-white dark:border-b-slate-800 border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-primary-600 dark:text-primary-400' : `text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700 border border-transparent ${activeVersionName !== null ? 'opacity-60' : ''}`}`}>TỔNG</button>
                            ...
                            <div className="space-y-10">
                                <div className="flex justify-end gap-3 no-print">
                                    <button 
                                        ...
                                        className="flex items-center gap-2 px-4 py-2 bg-primary-600 text-white text-sm font-black uppercase rounded-xl hover:bg-primary-700 transition-all shadow-lg shadow-primary-500/20 active:scale-95"
                                    >
