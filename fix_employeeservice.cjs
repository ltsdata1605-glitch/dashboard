const fs = require('fs');
const path = 'services/employeeService.ts';
...
content = content.replace(
    /if \(getHinhThucThanhToan\(row, productConfig\) === 'tra_gop'\)/g,
    "if (isTraCham)"
);

// We should also replace the parent group lookup to use _parentGroup
content = content.replace(
    /const parentGroup = getDisplayParentGroup\(maNhomHang, productConfig\);/g,
    "const parentGroup = row._parentGroup || getDisplayParentGroup(maNhomHang, productConfig);"
);
// In employeeService, it's getDisplayParentGroup which might be slightly different than getParentGroup. Let's verify getDisplayParentGroup.
