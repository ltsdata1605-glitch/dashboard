                            ...
                            <table className="w-full border-collapse border border-slate-200 dark:border-slate-700">
                                <thead className="sticky top-0 z-10">
                                    ...
                                    <tr>
                                        ...
                                        {providers.map(p => <th key={p.name} rowSpan={hidePercent ? 1 : undefined} colSpan={hidePercent ? 1 : 2} className={`px-1 py-1.5 text-center text-[11px] font-black uppercase tracking-wider text-sky-700 dark:text-sky-300 bg-sky-50 dark:bg-sky-900/30 border-r leading-tight align-middle ${hidePercent ? 'border-b-[3px] border-b-sky-400' : 'border-b border-slate-200'}`}>{p.shortName}</th>)}
                                        <th rowSpan={hidePercent ? 1 : 2} onClick={() => handleSort('totalDtSieuThi')} className="px-2 py-1.5 text-center text-[11px] font-black uppercase tracking-wider text-emerald-700 dark:text-emerald-300 bg-emerald-50 dark:bg-emerald-900/30 border-r border-b-[3px] border-b-emerald-400 cursor-pointer hover:bg-emerald-100 dark:hover:bg-emerald-900/40 leading-tight align-middle"><div>D.THU</div><div>THỰC</div></th>
                                    ...
                                    {!hidePercent && <tr className="bg-slate-50 dark:bg-slate-800/80">
                                        {providers.map(p => <React.Fragment key={p.name}><th className="px-1 py-1 text-center text-[11px] font-bold uppercase tracking-wider text-slate-500 border-r border-b-[3px] border-b-sky-400 cursor-pointer hover:bg-slate-100 transition-colors">DT</th><th className="px-1 py-1 text-center text-[11px] font-bold uppercase tracking-wider text-slate-500 border-r border-b-[3px] border-b-sky-400 cursor-pointer hover:bg-slate-100 transition-colors">%</th></React.Fragment>)}
                                    </tr>}
                                </thead>
