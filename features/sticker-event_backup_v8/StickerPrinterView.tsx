...
const resolvePagePrices = (page: StickerPage, priceSource: 'sale' | 'service') => {
    ...
    if (oldVal > 0 && newVal > 0) {
        ...
        if (diff > 0) {
            percent = `-${Math.round((diff / oldVal) * 100)}%`;
        }
    }

    if (priceSource === 'service') {
        newPrice = page.servicePrice || page.newPrice;
        const newServiceVal = Number(String(newPrice).replace(/\D/g, ''));
        if (oldVal > 0 && newServiceVal > 0) {
            if (newServiceVal * 1000 <= oldVal * 1.5 && newServiceVal < oldVal) {
                const correctedServiceVal = newServiceVal * 1000;
                const diff = oldVal - correctedServiceVal;
                if (diff > 0) {
                    percent = `-${Math.round((diff / oldVal) * 100)}%`;
                }
            } else {
                const diff = oldVal - newServiceVal;
                if (diff > 0) {
                    percent = `-${Math.round((diff / oldVal) * 100)}%`;
                }
            }
...
const renderPercentDiscount = (oldPriceStr: string, newPriceStr: string): string => {
    const oldVal = Number(String(oldPriceStr).replace(/\D/g, ''));
    let newVal = Number(String(newPriceStr).replace(/\D/g, ''));
    if (oldVal > 0 && newVal > 0) {
        if (newVal * 1000 <= oldVal * 1.5 && newVal < oldVal) {
            newVal = newVal * 1000;
        }
...
const renderAmountDiscount = (oldPriceStr: string, newPriceStr: string): React.ReactNode => {
    const oldVal = Number(String(oldPriceStr).replace(/\D/g, ''));
    let newVal = Number(String(newPriceStr).replace(/\D/g, ''));
    if (oldVal > 0 && newVal > 0) {
        if (newVal * 1000 <= oldVal * 1.5 && newVal < oldVal) {
        ...
        if (diff > 0) {
            ...
            return (
                <span className="discount-amount font-bold">
                    ...
                    <span className={`discount-unit ${unitClass}`}>{unit}</span>
                </span>
            );
        }
    }
...
const renderStickerHtml = (
    ...
    let header = headerTextContent || page.header;
    let subHeader = subHeaderTextContent || page.subHeader;
    let footer = footerTextContent || page.footer;
    ...
    const subHeaderHtml = stickerType === 'gio_vang' 
        ? `<div class="sub-header" style="font-size:${subHeaderTextSize}cqw">${subHeader || ''}</div>` : '';
    
    const displayPercent = discountDisplayMode === 'amount' 
        ? renderAmountDiscount(page.oldPrice, newPrice) 
        : `<span class="percent-text" style="font-size:${percentTextSize}cqw">${percent}</span>`;
    ...
    return `<div class="sticker-container" data-type="${stickerType}" style="background-image:url('${bgImage}');background-size:100% 100%;background-repeat:no-repeat;background-position:center;width:100%;aspect-ratio:197/285;position:relative;overflow:hidden;container-type:inline-size;font-family:Arial,sans-serif;">
        ${barcodeHtml}
        <div class="header-text" style="font-size:${headerTextSize}cqw">${header || ''}</div>
        ${subHeaderHtml}
        <div class="extra1">${displayPercent}</div>
        <div class="old" style="font-size:${oldPriceTextSize}cqw">${page.oldPrice}</div>
        <div class="name" style="font-size:${nameTextSize}cqw">${page.label}</div>
        <div class="newPrice" style="font-size:${newPriceTextSize}cqw">${newPrice}</div>
        <div class="footer-text" style="font-size:${footerTextSize}cqw">${footer || ''}</div>
    </div>`;
};
                    ...
                    {stickerMode === 'sticker' && (
                        <div className="flex items-center gap-1 ml-0.5 lg:ml-1 pl-1.5 lg:pl-2 border-l border-slate-200 dark:border-slate-700 animate-in fade-in slide-in-from-left-2 duration-200">
                            ...
                            <div className="flex items-center bg-white dark:bg-slate-800 border border-slate-200/50 dark:border-slate-700/50 rounded-full overflow-hidden shadow-sm h-[22px] lg:h-[26px]">
                                ...
                                <button onClick={resetActiveFontSize} className="px-1.5 lg:px-2 h-full flex items-center justify-center hover:bg-red-50 hover:text-red-600 dark:hover:bg-slate-700 border-l border-slate-100 dark:border-slate-700 text-slate-400 dark:text-slate-500 transition-colors" title="Đặt lại cỡ chữ mặc định">
                                    <RotateCcw size={12} />
                                </button>
                            </div>
                        </div>
