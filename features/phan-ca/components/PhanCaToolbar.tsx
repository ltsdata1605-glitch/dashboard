    ...
    return (
        <>
            ...
            <div className="lg:hidden sticky top-0 z-50 bg-white/95 dark:bg-slate-900/95 backdrop-blur-md border-b border-slate-200/60 px-3 py-2 flex items-center gap-2 overflow-x-auto no-scrollbar">
                <Button variant="ghost" onClick={onImportClick} className="bg-transparent hover:bg-transparent border-0 rounded-none h-8 w-auto p-0 text-inherit px-3 text-xs font-semibold text-white bg-indigo-600 hover:bg-indigo-700 rounded-md transition-colors flex items-center gap-1.5 shrink-0">
                ...
                <Button variant="ghost" onClick={onExportAll} className="bg-transparent hover:bg-transparent border-0 rounded-none h-8 w-auto p-0 text-inherit px-2.5 text-xs font-semibold text-slate-600 bg-white border border-slate-200 rounded-md transition-colors shrink-0" disabled={!hasStaff}>
                    Tất cả
                </Button>
                <Button variant="ghost" onClick={onExportWeekly} className="bg-transparent hover:bg-transparent border-0 rounded-none h-8 w-auto p-0 text-inherit px-2.5 text-xs font-semibold text-slate-600 bg-white border border-slate-200 rounded-md transition-colors shrink-0" disabled={!hasStaff}>
                    Tuần
                </Button>
                <Button variant="ghost" onClick={onExportIndividual} className="bg-transparent hover:bg-transparent border-0 rounded-none h-8 w-auto p-0 text-inherit px-2.5 text-xs font-bold text-white bg-slate-800 rounded-md transition-colors shrink-0" disabled={!hasStaff}>
                    Từng NV
                </Button>
                <Button variant="ghost" onClick={onExportExcel} className="bg-transparent hover:bg-transparent border-0 rounded-none h-8 w-auto p-0 text-inherit px-2.5 text-xs font-bold text-white bg-emerald-600 rounded-md transition-colors shrink-0" disabled={!hasStaff}>
                    Excel
                </Button>
                <Button variant="ghost" onClick={onExportGoogleSheet} className="bg-transparent hover:bg-transparent border-0 rounded-none h-8 w-auto p-0 text-inherit px-2.5 text-xs font-bold text-sky-700 bg-sky-50 rounded-md transition-colors shrink-0" disabled={!hasStaff} title="Xuất ra Google Sheet">
                    Sheet
                </Button>
            </div>
        </>
