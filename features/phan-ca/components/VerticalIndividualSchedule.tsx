      ...
      <table className="w-full border-collapse bg-white border border-slate-200 shadow-sm tabular-nums">
        <thead>
          <tr>
            ...
            <th colSpan={3} className="border-r-2 border-slate-400 py-3 bg-emerald-50 text-emerald-700 font-black text-[13px] uppercase tracking-widest border-b-2 !border-b-slate-400">Số Ngày SBH</th>
        ...
        <tbody className="divide-y divide-slate-100">
          {staff.schedule.slice(1).map((info, i) => {
            ...
            if (info.role.includes("(GH)")) {
                ...
                rolePill = <span className="inline-flex items-center justify-center px-2 py-0.5 rounded bg-sky-100 text-sky-800 font-bold text-[11px] border border-sky-200">THU NGÂN</span>;
            }
            ...
            return (
              <tr key={dayIndex} className={`${rowClass} hover:bg-slate-50 transition-colors border-b border-slate-300`}>
                ...
                <td className="py-3 px-6 text-center whitespace-nowrap">
                   {info.isManual && <span className="text-[11px] font-semibold text-sky-500 bg-sky-50 px-2 py-0.5 rounded border border-sky-100">Đã đổi ca</span>}
