    ...
    return (
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="bg-white dark:bg-slate-900 p-4 rounded-2xl shadow-sm border border-slate-200/50 flex flex-col gap-3">
                    <div className="flex items-center justify-between">
                         <div className="flex items-center gap-2 text-slate-500">
                            <BuildingStorefrontIcon className="h-5 w-5" />
                            <h3 className="text-xs font-black uppercase tracking-widest">Lọc Siêu thị</h3>
                         </div>
                         <span className="text-[10px] font-bold text-slate-400">{activeSupermarkets.length} đã chọn</span>
                    </div>
                    <div className="relative" ref={smRef}>
                        <button onClick={() => setIsSmFilterOpen(!isSmFilterOpen)} className="w-full flex items-center justify-between px-4 py-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-sm font-bold text-slate-700 dark:text-slate-200">
                            ...
                            <ChevronDownIcon className={`h-4 w-4 transition-transform ${isSmFilterOpen ? 'rotate-180' : ''}`} />
                        </button>
                        {isSmFilterOpen && (
                            <div className="absolute top-full left-0 right-0 mt-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl shadow-2xl z-[60] p-2 max-h-80 overflow-y-auto">
                                <div className="space-y-1">
                                    <div onClick={() => toggleSupermarket('all')} className="flex items-center justify-between p-2.5 rounded-lg cursor-pointer hover:bg-slate-50">
                                        <span className="text-xs font-black text-primary-600">CHỌN TẤT CẢ</span>
                                        <Switch checked={activeSupermarkets.length === supermarkets.length} onChange={() => {}} />
                                    </div>
                                    {supermarkets.map(sm => (
                                        <div key={sm} onClick={() => toggleSupermarket(sm)} className="flex items-center justify-between p-2.5 rounded-lg cursor-pointer hover:bg-slate-50">
                                            <span className="text-xs text-slate-600 dark:text-slate-400">{shortenSupermarketName(sm)}</span>
                ...
                <div className="bg-white dark:bg-slate-900 p-4 rounded-2xl shadow-sm border border-slate-200/50 flex flex-col gap-3">
                    <div className="flex items-center justify-between">
                         <div className="flex items-center gap-2 text-slate-500">
                            ...
                            <h3 className="text-xs font-black uppercase tracking-widest">Lọc Bộ phận</h3>
                         </div>
                         <span className="text-[10px] font-bold text-slate-400">{activeDepartments.includes('all') ? 'Tất cả' : `${activeDepartments.length} bộ phận`}</span>
                    </div>
                    <div className="relative" ref={deptRef}>
                        <button onClick={() => setIsDeptFilterOpen(!isDeptFilterOpen)} className="w-full flex items-center justify-between px-4 py-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-sm font-bold text-slate-700 dark:text-slate-200">
                            <span className="truncate flex-1 text-left">{activeDepartments.includes('all') ? 'TẤT CẢ BỘ PHẬN' : activeDepartments.join(', ')}</span>
                            <ChevronDownIcon className={`h-4 w-4 transition-transform ${isDeptFilterOpen ? 'rotate-180' : ''}`} />
                        </button>
                        {isDeptFilterOpen && (
                            <div className="absolute top-full left-0 right-0 mt-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl shadow-2xl z-[60] p-2 max-h-80 overflow-y-auto">
                                <div className="space-y-1">
                                    <div className="flex items-center justify-between p-2.5 rounded-lg hover:bg-slate-50 cursor-pointer" onClick={() => toggleDepartment('all')}>
                                        <span className="text-xs font-black text-primary-600">TẤT CẢ</span>
                                        <Switch checked={activeDepartments.includes('all')} onChange={() => {}} />
                                    </div>
                                    {departmentOptions.map(dept => (
                                        <div key={dept} className="flex items-center justify-between p-2.5 rounded-lg hover:bg-slate-50 cursor-pointer" onClick={() => toggleDepartment(dept)}>
                                            <span className="text-xs text-slate-600 dark:text-slate-300">{dept}</span>
