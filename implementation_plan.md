...
### Mục 58.1 — Bug tái phát: "Preview chuẩn, nhưng bấm In (Chrome print dialog) vẫn sai cỡ chữ" (2026-07-28, phiên sau)

**Trạng thái kế thừa khi bắt đầu phiên này**: `git status` cho thấy `pageHtmlUtils.ts` và `stickerPreviewStyles.ts` đã có thay đổi CHƯA COMMIT từ phiên trước (không phải do tôi tạo), đổi hướng từ cách `cqw` ở Mục 58 sang đơn vị `mm` tuyệt đối — lý do ghi trong comment code: "Chrome print engine không giải quyết cqw/cqi nhất quán khi container bị ép cứng cả width lẫn height", tức là dù Mục 58 đã verify bằng Playwright `page.pdf()` là đúng, thực tế người dùng bấm In thật qua hộp thoại Chrome (ảnh chụp gửi kèm) vẫn bị sai — nghi vấn khác biệt giữa `page.pdf()` (Playwright) và hộp thoại "In" tương tác thật của Chrome đối với `cqw`. Phần chuyển `pageHtmlUtils.ts` sang inline `style="font-size:...mm"` (tính động từ đúng cỡ chữ người dùng cấu hình qua `drawTitleSize`/`drawContentTopLeftSize`/...) là **đúng hướng và giữ nguyên**.

**Bug trong phần kế thừa (dở dang)**: `stickerPreviewStyles.ts` có thêm khối `@media print { ... }` ghi đè `font-size` bằng giá trị `mm` **cứng** (hardcode) kèm `!important` cho 7 field (title, content-top-left, code-left/right, content-bottom-left/right, footer). Vì `!important` trong stylesheet luôn thắng inline style không có `!important`, khối này **đè mất** toàn bộ cỡ chữ động vừa tính ở trên khi in — bản in luôn ra đúng 1 bộ cỡ chữ cố định, bất kể người dùng chỉnh gì trong UI (ví dụ ô "Cỡ chữ Giải thưởng trái"). Đối chiếu số liệu: giá trị cứng cho content-top-left là `4.62mm` (= 2.2cqw) trong khi mặc định thật trên preview (`DrawTicketBlock.tsx`) là `3.5cqw` (≈7.35mm) — lệch ngay cả với giá trị mặc định, không cần người dùng chỉnh gì. Title cứng `6.30mm` (3.0cqw) trong khi mặc định preview `2.5cqw` (≈5.25mm) — in ra to hơn preview ~20%. Đây khớp chính xác với triệu chứng ảnh người dùng gửi: preview đúng nhưng in ra to/lệch.

**Đã sửa**: Xoá 7 dòng `font-size: ...mm !important` bị hardcode trong khối `@media print` (`stickerPreviewStyles.ts`), giữ lại `padding`/`line-height` (không có inline tương ứng nên vẫn cần, do `cqw` bị tắt khi in). `pageHtmlUtils.ts` giữ nguyên như phiên trước (không đụng tới).

**Kiểm thử**: `npm run check` PASS (typecheck + eslint + build + lint-ratchet, 0 lỗi mới). Có thử dựng lại HTML in thật bằng cách bundle trực tiếp `generateDrawPagesHtml()`/`getStickerPreviewStyles()` qua esbuild rồi render trong Chromium (`page.pdf`/screenshot ở print-media) nhưng dữ liệu mẫu tự tạo (không có ảnh nền thật, text mẫu tự bịa dài hơn thực tế) gây chồng chữ giữa các phiếu — không phản ánh đúng bug đang sửa nên bỏ kết quả đó, không dùng làm bằng chứng. Việc verify E2E qua app thật (login + bấm nút In) bị người dùng từ chối cho chạy tự động trong phiên này. **Chưa xác minh trực quan** bản sửa lần này qua PDF/máy in thật — khuyến nghị mạnh người dùng tự bấm "BẤM ĐỂ IN" rồi kiểm tra hộp thoại in Chrome (hoặc lưu PDF) sau khi cập nhật code, đối chiếu với preview trong app.
