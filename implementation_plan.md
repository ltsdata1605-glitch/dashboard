...
**Cập nhật (2026-08-08)**: bộ lọc Mã Kho + Tổng cho bảng dọc đã HOÀN THÀNH — nhưng qua 1 cơ chế khác, gọn hơn dự kiến ban đầu ở trên. Thay vì 2 cơ chế tách rời (`localKhoFilter` lọc data + `showTotalColumn` toggle riêng, như ghi chú cũ mô tả), giải pháp thực tế dùng 1 dropdown DUY NHẤT `verticalKhoFilter`/`handleVerticalKhoFilterChange` (option `verticalOptions = ['TỔNG', ...khoOptions]`, gắn UI qua `MultiSelectDropdown iconOnly` cạnh view Dọc), coi "TỔNG" như 1 lựa chọn trong cùng danh sách thay vì nút bật/tắt riêng — dẫn xuất `displayedVerticalData`/`showVerticalTotal` được dùng xuyên suốt header 2 tầng + render body bảng dọc. Đã dọn sạch code chết của thiết kế cũ (`localKhoFilter`/`updateLocalKhoFilter`/`showTotalColumn`/`toggleShowTotal` — setter không hề được gọi ở đâu, xác nhận qua grep) khỏi `WarehouseSummary.tsx`.
...
**Regression phát hiện + fix (2026-08-08)**: đo lại tốc độ thật bằng Playwright (CPU/network throttle mô phỏng mobile, cold start IndexedDB) phát hiện lỗi NGHIÊM TRỌNG do chính fix #1 ở trên gây ra — mọi lần MỞ LẠI app với dữ liệu đã lưu sẵn (kịch bản dùng hàng ngày, không phải lần đầu) đều báo lỗi giả "Không tìm thấy dữ liệu hợp lệ trong tệp đã chọn!" rồi kẹt vĩnh viễn ở màn hình upload dù dữ liệu vẫn còn nguyên trong IndexedDB — tái hiện 5/5 lần, không phụ thuộc throttle hay số dòng dữ liệu.

*Root cause*: `setAppState('processing')` (dòng ~178 cũ) bị tách RIÊNG khỏi `startTransition(() => setOriginalData(srcData))` — 2 state update lệch độ ưu tiên (1 cao/đồng bộ, 1 thấp/có thể trễ) nên `appState` chuyển sang `'processing'` TRƯỚC KHI `originalData` thực sự cập nhật. Effect "Central Data Processing" (dòng ~728) đọc phải `originalData.length === 0` (cũ, chưa kịp cập nhật) trong khi `appState === 'processing'` → báo lỗi giả + set `appState` về `'upload'`. Vì `originalData` KHÔNG nằm trong dependency array của effect đó (dòng ~775, đây là thiết kế có chủ đích từ trước, không phải lỗi), khi `originalData` thật sự cập nhật sau đó, effect không chạy lại — app kẹt vĩnh viễn.

*Fix*: gộp `setAppState('processing')` VÀO CHUNG `startTransition` với `setOriginalData(srcData)` (giống hệt pattern effect đồng bộ dữ liệu Kho ở dòng ~452-455 — vốn KHÔNG bị lỗi này chính vì đã gộp chung sẵn) — khôi phục tính atomic giữa 2 update trong khi vẫn giữ nguyên lợi ích không chặn main thread của `startTransition`. Xác minh: `tsc --noEmit` sạch, `vite build` thành công, `lint-ratchet` không phát sinh vi phạm mới. Chunking fix (#3) được xác nhận hoạt động đúng qua build thật (`vendor-charts` không còn trong `modulepreload` list). Commit `4fc3f93e`, đã push + đã deploy production (bug cũ đang chạy thật trên gh-pages tại thời điểm phát hiện — deploy lại ngay sau khi fix).

**Đo lại sau fix (2026-08-08, Playwright thật trên production build `vite preview`, 4x CPU + network throttle mô phỏng mobile, dữ liệu giả sinh từ mã ngành hàng thật lấy trực tiếp từ Google Sheet cấu hình)**:
- **Đúng chức năng**: ĐÃ HẾT lỗi kẹt màn hình upload — 6/6 lần mở lại app (3 lần x 20k dòng, 3 lần x 50k dòng) đều vào được dashboard đúng, không còn báo lỗi giả. Trước fix: 5/5 lần lỗi.
- **Độ mượt (rAF gap trong lúc xử lý)**: CHƯA đạt mục tiêu "mượt" như kỳ vọng — vẫn còn 1-3 khoảng đứng hình dài trong mỗi lần tải: ~950ms-1.1s/lần (20k dòng), ~1.8-3.1s/lần (50k dòng), chiếm 70-80% tổng thời gian xử lý. Tổng thời gian từ lúc hiện "32%" tới lúc vào dashboard: ~2.6-2.9s (20k dòng), ~6.3-6.4s (50k dòng).
- **Nguyên nhân độ mượt chưa đạt**: `startTransition` chỉ sửa đúng lỗi tranh chấp độ ưu tiên giữa 2 state update (mục tiêu ban đầu) — KHÔNG có khả năng ngắt quãng một vòng lặp đồng bộ (`for`) đang chạy DỞ CHỪNG bên trong 1 `useMemo` (`rbacData`/`uniqueFilterOptions`/`allUnconfiguredGroups`, dòng ~178-186/791). React scheduler chỉ có thể trì hoãn/ngắt quãng GIỮA các đơn vị render (fiber), không thể chen vào giữa 1 vòng lặp JS thuần đang chạy. Muốn đạt độ mượt thật (gap <200ms) cần chuyển hẳn phần tính toán nặng này ra Worker (đã có sẵn Worker cho bước PROCESS, xem `services/analytics.worker.ts`/dòng ~710-725) hoặc chia nhỏ vòng lặp bằng time-slicing thủ công (`scheduler.postTask`) — đây là việc LỚN hơn, thuộc nhóm "cố ý chưa làm" ở trên, chưa làm trong đợt này.

## Mục 65b — Chuyển rbacData/uniqueFilterOptions/allUnconfiguredGroups sang Worker (theo yêu cầu tiếp tục ở trên)

**Đã làm** (kế hoạch đầy đủ tại `~/.claude/plans/wondrous-zooming-moler.md`): port nguyên văn 3 hàm sang `utils/dataUtils.ts` (`computeRbacFilteredData`/`computeUniqueFilterOptions`/`computeUnconfiguredGroups`, đã đối chiếu byte-for-byte với bản gốc — khớp tuyệt đối, chỉ khác đúng phần tham số hoá đã dự tính); mở rộng message `SET_DATA` của `services/analytics.worker.ts` để tính cả 3 hàm trong Worker; tái cấu trúc `hooks/useDataManagement.ts` — xoá 3 `useMemo`, thay bằng `useState` + 1 effect gửi `SET_DATA`, gộp `onmessage` về 1 chỗ, thêm cơ chế **generation counter** (`dataGenerationRef`/`workerCachedGeneration`) chặn effect "Central Data Processing" gửi `PROCESS` cho tới khi Worker xác nhận đã cache đúng dữ liệu cho generation hiện tại — tránh lặp lại kiểu lỗi ngầm định thứ tự effect đã gây regression ở commit `4fc3f93e`.

**Kết quả verify (Playwright thật, production build `vite preview`, throttle mobile, 20k/50k dòng)**:
- **Đúng chức năng — ĐẠT**: reload-regression 6/6 lần đúng (không lặp lại lỗi cũ). Test race-condition mới (2 lần đổi `originalData` liên tiếp nhanh, dispatch đồng thời) 3/3 lần không sinh trạng thái lẫn/cũ — `uniqueFilterOptions`/KPI luôn khớp nhau, không rò dữ liệu giữa 2 lần tải. Đổi filter đơn thuần (không đổi data) xác nhận KHÔNG gửi lại `SET_DATA` (chỉ `PROCESS` chạy nhanh như cũ). Không có `DataCloneError`/lỗi console nào.
  - Lưu ý coverage: agent verify KHÔNG ép được kịch bản "round-trip SET_DATA lần 1 CHƯA XONG mà lần 2 đã gửi" (do `handleFileProcessing` đã tự khoá UI/unmount input rất nhanh) — cơ chế generation-gate đã được review tĩnh kỹ và cho là đúng, nhưng chưa có bằng chứng runtime ép buộc đúng nhánh "bỏ qua response cũ" thực thi. Rủi ro thấp (logic đơn giản, đã review kỹ) nhưng ghi nhận là chưa 100% closed-loop.
- **Độ mượt — MỤC TIÊU BAN ĐẦU KHÔNG ĐẠT, nhưng đã xác định đúng nguyên nhân thật**: gap tối đa gần như KHÔNG ĐỔI so với trước khi làm Mục 65b (20k: ~1.1s cả trước/sau; 50k: ~2.4-3.1s cả trước/sau). Đo trực tiếp bằng cách đối chiếu timestamp log Worker với rAF-gap: `SET_DATA` (đúng phần đã chuyển sang Worker ở Mục 65b) giờ chỉ mất ~165ms (20k) / ~610ms (50k) round-trip — ĐÃ đúng như kỳ vọng, không còn là phần chặn chính. **Khoảng đứng hình lớn thật ra đến từ 2 nguồn KHÁC, chưa từng bị đụng tới ở cả Mục 65 lẫn 65b**: (1) bước `PROCESS`/`applyFiltersAndProcess` — tuy ĐÃ chạy trong Worker từ trước, bản thân phép tính này với 50k dòng mất tới ~3.1s trong Worker; (2) React render TOÀN BỘ dashboard (nhiều bảng + biểu đồ recharts) ngay sau khi `PROCESS_SUCCESS` về — đây là 1 lượt commit đồng bộ, to, KHÔNG thể tránh bằng Worker (Worker không đụng được DOM) và cũng chưa từng được `startTransition`/chia nhỏ.
- **Kết luận**: Mục 65b tự nó ĐÚNG và AN TOÀN (đã port chuẩn, verify kỹ, giảm được phần việc đã nhắm tới) nhưng KHÔNG giải quyết được vấn đề "đứng hình" ban đầu user báo — phần đó nằm ở chỗ khác (thời gian tính `applyFiltersAndProcess` cho tập dữ liệu lớn + chi phí render toàn bộ dashboard). Đây là 1 phát hiện MỚI, cần 1 đợt điều tra/kế hoạch riêng nếu muốn tiếp tục đạt "NHANH MƯỢT" thật sự — không phải việc chỉnh nhỏ như 2 đợt trước.

## Mục 65c — 3 fix nhắm đúng 3 nguyên nhân thật (2026-08-09, commit `2941417d`/`f403086e`/`65662fa2`)

Khảo sát sâu (2 Explore agent song song) xác định đúng 3 nguồn lãng phí thật, đã sửa và verify từng nhóm riêng (xem `~/.claude/plans/wondrous-zooming-moler.md` để biết thiết kế đầy đủ):

1. **`FilterBar.tsx`/`FilterSection.tsx`** — quét toàn bộ `originalData` (tới 50k dòng) trùng lặp để dựng dropdown tuần/tháng, không trì hoãn. Gộp về 1 chỗ tính trong `hooks/useDataManagement.ts`, trì hoãn bằng pattern có sẵn ở `useWarehouseLogic.ts`. Verify: Playwright xác nhận dropdown vẫn đúng giá trị.
2. **`useSummaryTableLogic.ts`** — tính lại toàn bộ bảng tổng hợp trên main thread dù Worker đã tính y hệt. Thêm short-circuit dùng thẳng `processedData.summaryTable` khi chưa có filter cục bộ nào. Verify: Playwright trực tiếp xác nhận 3 kịch bản (tải mới/lọc/xoá lọc) đều cho số liệu đúng, khớp nhau khi nên khớp.
3. **`calculateRowMetrics()`/`getParentGroup()`** bị gọi lại 3-4 lần/dòng trong `employeeService.ts`/`summaryService.ts` — đổi sang đọc `row._metrics`/`row._parentGroup` đã tính sẵn, đúng pattern `kpiService`/`trendService`/`industryService` đã dùng. Sửa kèm lỗi thứ tự thực thi (`calculateWarehouseSummary` chạy trước `processDataForPeriod`, không đọc được cache). Verify: script Node độc lập đối chiếu JSON kết quả trước/sau trên dữ liệu giả 20k/50k dòng — **0 sai số** ở mọi kịch bản filter + cache-hit/cache-miss.

**Đo lại timing tổng thể cả 3 nhóm cộng dồn (Playwright, production build `vite preview`, throttle mobile, cùng phương pháp các đợt trước)**:
- 20k dòng: gap tối đa 1162-1199ms — **gần như không đổi** so với Mục 65b (1068-1101ms), thậm chí hơi cao hơn (trong ngưỡng nhiễu đo run-to-run ~10-20% đã ghi nhận từ trước).
- 50k dòng: gap tối đa 2408-2474ms — cải thiện nhẹ, nằm ở đầu thấp của khoảng Mục 65b (2387-3077ms), không phải bước nhảy rõ rệt.
...
## Mục 65e — Tiếp tục dọn 3 mảng dòng thô còn sót (2026-08-09)

**Đo profiling tạm thời** (thêm `performance.now()` marker trong Worker, đo xong revert ngay — không phải thay đổi lâu dài) phát hiện: bước SET_DATA có round-trip ~540-660ms nhưng phần TÍNH TOÁN thật bên trong Worker (RBAC filter + uniqueFilterOptions + allUnconfiguredGroups) chỉ mất **~46ms** — phần lớn round-trip là chi phí truyền `originalData` (50k dòng) qua lại, không tránh được vì Worker vẫn cần dữ liệu này cho bước PROCESS. Riêng bước PROCESS: `applyFiltersAndProcess` tự nó mất ~400-478ms, phần còn lại (~400-450ms trong tổng ~880-925ms round-trip) là chi phí clone/truyền payload trả về.

Đồng thời phát hiện `unshippedOrders`/`uncollectedOrders`/`debtOrders` (processedData) VẪN đang gửi dạng mảng dòng thô đầy đủ (~5.600 dòng trong tập test 50k) — cùng loại vấn đề đã sửa ở Mục 65d cho 4 mảng khác. Đã mở rộng ĐÚNG pattern đã verify: `unshippedOrders`/`debtOrders` giờ lọc trực tiếp từ `filteredValidSalesData` đã có sẵn trên main thread (rẻ, không cần tính thêm); `uncollectedOrders` lọc từ `mainPeriodData` bằng hàm mới tách export `isUncollectedOrder()` (`utils/dataUtils.ts`, port nguyên văn từ nhánh "chưa thu" trong `processDataForPeriod`). Worker không còn gửi 3 mảng này nữa.

**Verify**: `applyFiltersAndProcess` đối chiếu số liệu trước/sau khớp 100% (thuần trích xuất hàm). Playwright: reload 3/3, filter cục bộ, race-condition — tất cả kết quả giống hệt trước khi sửa (không regression).

**Kết quả đo lại — bất ngờ, cần ghi nhận trung thực**: payload GẦN NHƯ KHÔNG ĐỔI (14.861.117 → 14.861.057 byte, chênh 60 byte, coi như 0%). Đào sâu bằng cách đo kích thước JSON từng field riêng lẻ phát hiện lý do: **`summaryTable` chiếm 12.707.922 byte (85,5%)** và **`warehouseSummary` chiếm 1.950.536 byte (13,1%)** của tổng 14,86MB — cộng lại 98,6%. 3 mảng vừa xoá (`unshippedOrders`/`debtOrders`/`uncollectedOrders`, ~5.600 dòng) chỉ là phần rất nhỏ còn lại, không đáng kể.
...
**Root cause** (xác nhận qua đọc code, không đoán): `contexts/AuthContext.tsx` có cơ chế "Ultra-Fast Boot" — đọc `role`/`status`/`departmentId` đã lưu cache ở IndexedDB (từ lần mở app trước) và tắt spinner loading NGAY LẬP TỨC nếu có `role` + `status` trong cache, **bất kể giá trị đó là gì**. Nếu cache còn lưu `status` cũ (`'pending'`/`'new'`/`'expired'` — vd còn sót lại từ trước khi tài khoản này được công nhận Super Admin, hoặc dữ liệu cache stale vì lý do khác), app render NGAY `PendingApprovalView` bằng dữ liệu cache cũ đó — rồi mới tự sửa lại vài trăm ms sau khi `resolveSession()` (gọi mạng, LUÔN tự phục hồi đúng quyền cho email Super Admin — `functions/src/session.ts:81-88`) chạy xong và ghi đè state đúng. Đúng khớp mô tả "nháy màn rồi mới vào trang".

**Fix**: `contexts/AuthContext.tsx` — điều kiện tắt spinner sớm đổi từ `if (r && s)` (bất kỳ giá trị nào) thành yêu cầu đúng 2 điều kiện App.tsx thật sự dùng để vào thẳng dashboard: `status === 'approved'` VÀ (`role === 'admin'` HOẶC có `departmentId` cache hợp lệ). Không bao giờ tắt spinner sớm cho 1 trạng thái cache sẽ dẫn tới màn hình KHÁC dashboard.

**Verify**: không test được qua Playwright (cần đăng nhập Google thật, không giả lập được bằng Trial Mode) — verify bằng cách liệt kê hết 120 tổ hợp `role × status × departmentId` khả dĩ, so sánh quyết định "tắt spinner sớm" của code cũ/mới với quyết định thật của `App.tsx`: **code cũ có 40/120 tổ hợp sai** (xác nhận đây là lỗi thật, không phải hiểu lầm), **code mới có 0/120 vi phạm**. `tsc`/`eslint`/`build`/`lint-ratchet` sạch.
