...
const UnshippedOrdersModal: React.FC<UnshippedOrdersModalProps> = ({ isOpen, onClose, onExport, onlyOverdue }) => {
    ...
    // cả", hoặc trước khi xuất ảnh) — tránh mount hàng nghìn <tr> vào DOM ngay khi mở modal (cấp
    // "creator" luôn `open` sẵn nên toàn bộ customer bên trong cũng bị mount theo nếu không có cơ
    // chế này, dù đang đóng bằng CSS mặc định của <details>). Chỉ THÊM, không bao giờ gỡ khỏi Set
    // sau khi đã render — tránh phức tạp/giật khi đóng lại.
    ...
    // xuất ảnh qua forceOpenDetails) — nếu không, DOM tại thời điểm đó vẫn còn thiếu bảng đơn hàng
    // của các customer chưa từng được user tự mở.
    ...
    const toggleAllDetails = () => {
        ...
        if (nextState) {
            // Đảm bảo bảng đơn hàng của MỌI customer đã render trước khi mở native <details> —
            // nếu không, các customer chưa từng được user tự mở sẽ mở ra rỗng.
            renderCustomersSync(getAllCustomerIds());
        }
    ...
    const { industryDataForDisplay, totalUnshippedRevenue, totalUnshippedRevenueQD, creatorData } = useMemo(() => {
        ...
        finalUnshippedOrders.forEach(order => {
            ...
            totalUnshippedRevenue += revenue;
            totalUnshippedRevenueQD += revenueQD;
        });
