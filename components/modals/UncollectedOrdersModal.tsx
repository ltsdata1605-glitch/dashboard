...
const UncollectedOrdersModal: React.FC<UncollectedOrdersModalProps> = ({ isOpen, onClose, onExport, onExportSheet, onlyOverdue }) => {
    ...
    const handleExportAll = async () => {
        ...
        if (elementToExport) {
            ...
            await onExport(elementToExport, `don-hang-chua-thu-all.png`, { forceOpenDetails: true, forcedWidth: 960 });
    ...
    const handleBatchExport = async () => {
        ...
        for (let i = 0; i < creatorData.length; i++) {
            ...
            if (creatorElement) {
                const filename = `chua-thu-${creator.name.replace(/[^a-zA-Z0-9]/g, '_')}.png`;
    ...
    const handleExportCreator = async (e: React.MouseEvent, creatorName: string) => {
        ...
        if (creatorElement) {
            ...
            const filename = `chua-thu-${creatorName.replace(/[^a-zA-Z0-9]/g, '_')}.png`;
            await onExport(creatorElement, filename, {
                forceOpenDetails: true,
                forcedWidth: 960,
            });
