...
import {
  Activity, AlertCircle, AlertTriangle, ArchiveRestore, ArrowRight, Award, BarChart2, BarChart3, Bell, BellOff,
  ...
  Contact, Copy, Cpu, CreditCard, Database, DollarSign, Download, DownloadCloud, Edit3, ExternalLink, Eye, EyeOff, FastForward, FileCheck,
  FileSpreadsheet, FileText, FileUp, Filter, GalleryHorizontalEnd, GanttChartSquare, Glasses,
  GripHorizontal, GripVertical, History, Images, Inbox, Info, Layers, LayoutDashboard, LayoutGrid, LayoutList,
  LayoutTemplate, Link, List, Loader2, LogOut, MapPin, Maximize2, Megaphone, MessageCircle, Minimize2,
  Package, PackageX, Paintbrush, Palette, Pencil, Percent, PieChart, Play, Plus, PlusCircle, Receipt, RefreshCcw,
  RefreshCw, RotateCcw, Save, ScanLine, Search, SearchX, Settings, Settings2, Share2, Sheet, Shield,
  ShieldCheck, ShoppingBag, Sigma, Sparkles, Square, Table, Table2, Tag, Target, Trash2, Trophy, TrendingUp, Truck, Type,
  Upload, User, UserCheck, UserMinus, Users, UsersRound, WalletCards, Warehouse, X, Zap,
} from 'lucide-react';
...
const ICON_MAP: Record<string, LucideIcon> = {
  'activity': Activity, 'alert-circle': AlertCircle, 'alert-triangle': AlertTriangle, 'archive-restore': ArchiveRestore,
  'arrow-right': ArrowRight, 'award': Award, 'bar-chart-2': BarChart2, 'bar-chart-3': BarChart3, 'bell': Bell, 'bell-off': BellOff,
  ...
  'columns': Columns, 'columns-2': Columns2, 'contact': Contact, 'copy': Copy, 'cpu': Cpu, 'credit-card': CreditCard,
  'database': Database, 'dollar-sign': DollarSign, 'download': Download, 'download-cloud': DownloadCloud, 'edit-3': Edit3,
  'external-link': ExternalLink, 'eye': Eye, 'eye-off': EyeOff, 'fast-forward': FastForward, 'file-check': FileCheck,
  ...
  'layout-dashboard': LayoutDashboard, 'layout-grid': LayoutGrid, 'layout-list': LayoutList, 'layout-template': LayoutTemplate,
  ...
  'minimize-2': Minimize2, 'package': Package, 'package-x': PackageX, 'paintbrush': Paintbrush, 'palette': Palette,
  'pencil': Pencil, 'percent': Percent, 'pie-chart': PieChart, 'play': Play, 'plus': Plus,
  'plus-circle': PlusCircle, 'receipt': Receipt, 'refresh-ccw': RefreshCcw, 'refresh-cw': RefreshCw,
  ...
  'sheet': Sheet, 'shield': Shield, 'shield-check': ShieldCheck, 'shopping-bag': ShoppingBag, 'sigma': Sigma,
  'sparkles': Sparkles, 'square': Square, 'table': Table, 'table-2': Table2, 'tag': Tag,
  'target': Target, 'trash-2': Trash2, 'trophy': Trophy, 'trending-up': TrendingUp, 'truck': Truck, 'type': Type,
  'upload': Upload, 'user': User, 'user-check': UserCheck, 'user-minus': UserMinus,
  'users': Users, 'users-round': UsersRound, 'wallet-cards': WalletCards, 'warehouse': Warehouse, 'x': X, 'zap': Zap,
};
