...
const EmployeeAnalysisContent: React.FC<EmployeeAnalysisContentProps> = React.memo(({
    ...
    const handleEditCustomTab = React.useCallback((tabId: string, mode: 'detail' | 'efficiency') => {
        ...
        if (!tab && tabId === 'doanhThu') {
            ...
            tab = { id: 'spChinh', name: 'SP Chính', order: 0, columns: [ { id: 'slICT', name: 'ICT', type: 'quantity', filters: { selectedIndustries: ['ICT'], selectedSubgroups: [], selectedManufacturers: [], productCodes: [] } }, { id: 'slCE_main', name: 'CE', type: 'quantity', filters: { selectedIndustries: ['CE'], selectedSubgroups: [], selectedManufacturers: [], productCodes: [] } }, { id: 'slGiaDung_main', name: 'ĐGD', type: 'quantity', filters: { selectedIndustries: ['Gia dụng'], selectedSubgroups: [], selectedManufacturers: [], productCodes: [] } }, { id: 'slSPChinh_Tong', name: 'Tổng', type: 'quantity', filters: { selectedIndustries: [], selectedSubgroups: [], selectedManufacturers: [], productCodes: [] } } ] };
        }
        if (tab) {
                        ...
                        <div className="flex justify-between items-center mb-3 sm:mb-6">
                            ...
                            <div className="px-1.5 sm:px-6 py-1 sm:py-2 border-b border-slate-100 dark:border-slate-800 bg-slate-50/30 dark:bg-slate-900/30 hide-on-export overflow-x-auto rounded-lg sm:rounded-xl">
                                <div className="flex items-center gap-0.5 sm:gap-1.5 flex-wrap">
                                    <Button variant="unstyled" size="none" onClick={() => setModalState({ type: 'CREATE_TABLE', data: { tabId: customTab.id }})} title="Tạo Bảng Thi Đua Mới" className="p-1.5 sm:p-2 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg sm:rounded-xl transition-all">
                                        <Icon name="plus" size={3.5} className="sm:hidden" /><Icon name="plus" size={5} className="hidden sm:block" />
                                    </Button>
                                    <Button variant="unstyled" size="none" onClick={() => setModalState({ type: 'EDIT_TAB', data: { tabId: customTab.id, initialName: customTab.name, initialIcon: customTab.icon }})} title="Sửa Tên Tab" className="p-1.5 sm:p-2 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg sm:rounded-xl transition-all">
                                        <Icon name="edit-3" size={3.5} className="sm:hidden" /><Icon name="edit-3" size={5} className="hidden sm:block" />
                                    </Button>
                                    <Button variant="unstyled" size="none" onClick={() => setModalState({ type: 'CONFIRM_DELETE_TAB', data: { tabId: customTab.id, tabName: customTab.name }})} title="Xóa Tab" className="p-1.5 sm:p-2 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-lg sm:rounded-xl transition-all">
                                    ...
                                    <Button variant="unstyled" size="none" onClick={handleMainExport} disabled={isExporting} title="Xuất Ảnh Tab" className="p-1.5 sm:p-2 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg sm:rounded-xl transition-all">
