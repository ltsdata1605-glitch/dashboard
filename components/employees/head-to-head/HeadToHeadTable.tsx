            ...
            <div className="overflow-x-auto custom-scrollbar flex-grow border border-slate-200 dark:border-slate-700">
                <table className="w-full text-left border-collapse">
                    <thead className="sticky top-0 z-20 bg-white dark:bg-slate-900 border-b-[3px] !border-b-slate-300 dark:!border-b-slate-600">
                        <tr>
                            <th colSpan={2} onClick={() => handleSort('name')} className="px-2 sm:px-3 py-1 sm:py-2 text-left text-[9px] sm:text-[11px] font-bold text-indigo-700 dark:text-indigo-300 bg-indigo-50 dark:bg-indigo-900/30 uppercase tracking-wider cursor-pointer select-none min-w-[110px] sm:min-w-[150px] sticky left-0 z-20 border-r border-slate-200 dark:border-slate-700 hover:bg-indigo-100 dark:hover:bg-indigo-900/50 transition-colors">
                            ...
                            {processedData.dateHeaders.map(date => {
                                ...
                                return (
                                    <th key={date.toISOString()} onClick={() => handleSort(dateKey)} className={`px-1 sm:px-2 py-1 sm:py-2 text-center text-[9px] sm:text-[11px] font-bold ${metricColor.text} uppercase tracking-wider cursor-pointer select-none ${metricColor.bg} border-r border-slate-200 dark:border-slate-700 ${isSorted ? '!bg-indigo-50 dark:!bg-indigo-900/30' : ''} hover:brightness-95 transition-all`}>
                            ...
                            <th onClick={() => handleSort('total')} className="px-1.5 sm:px-3 py-1 sm:py-2 text-center text-[9px] sm:text-[11px] font-bold text-indigo-700 dark:text-indigo-400 uppercase tracking-wider cursor-pointer select-none bg-indigo-50/50 dark:bg-indigo-900/20 border-r border-slate-200 dark:border-slate-700 hover:bg-indigo-100 transition-colors">
                            ...
                            <th onClick={() => handleSort('daysWithNoSales')} className="px-1.5 sm:px-3 py-1 sm:py-2 text-center text-[9px] sm:text-[11px] font-bold text-rose-700 dark:text-rose-400 uppercase tracking-wider cursor-pointer select-none bg-rose-50/50 dark:bg-rose-900/20 hover:bg-rose-100 transition-colors">
                                NS
                        ...
                        {processedData.sortedDepartments.map((department, deptIndex) => {
                            ...
                            return(
                                <React.Fragment key={department}>
                                     <tr>
                                        <td colSpan={100} className={`px-2 sm:px-3 py-1 sm:py-1.5 ${DEPT_COLORS[deptIndex % DEPT_COLORS.length].strip} border-y border-slate-200 dark:border-slate-700 sticky left-0 z-10`}>
                                    ...
                                    {rows.map((row, rowIndex) => {
                                        return (
                                            <tr key={row.name} className={`group border-b border-slate-200 dark:border-slate-700 transition-colors duration-100 hover:bg-slate-100 dark:hover:bg-slate-800 ${rowIndex % 2 === 0 ? 'bg-white dark:bg-slate-900' : 'bg-slate-50 dark:bg-slate-800'}`}>
                                                ...
                                                <td className="px-1.5 sm:px-3 py-1 text-left sticky left-8 bg-inherit z-10 border-r border-slate-200 dark:border-slate-700">
                                                ...
                                                {processedData.dateHeaders.map(date => {
                                                    ...
                                                    return (
                                                        <td key={dateKey} className={`px-1 sm:px-2 py-1 text-center text-[11px] sm:text-[13px] font-bold text-slate-600 dark:text-slate-300 tabular-nums border-r border-slate-200 dark:border-slate-700 ${metricColor.bg}`}>
                                    ...
                                    {deptTotalData && processedData.sortedDepartments.length > 1 && (
                                        <tr className="bg-slate-50 dark:bg-slate-800/50 border-t border-slate-200 dark:border-slate-700 font-bold">
                                            <td colSpan={2} className="px-2 sm:px-3 py-1 sm:py-1.5 text-left text-[9px] sm:text-[11px] uppercase font-black text-slate-500 dark:text-slate-400 sticky left-0 bg-slate-50 dark:bg-slate-800 z-10 border-r border-slate-200 dark:border-slate-700 tracking-widest">Tổng {department}</td>
                                            {processedData.dateHeaders.map(date => {
                                                const dateKey = toLocalISOString(date);
                                                return (
                                                    <td key={dateKey} className={`px-1 sm:px-2 py-1 sm:py-1.5 text-center text-[11px] sm:text-[13px] font-black tracking-wide border-r border-slate-200 dark:border-slate-700 ${metricColor.text} ${metricColor.bg}`}>
                                                        {formatValue(deptTotalData.daily.get(dateKey) || 0)}
                                                    </td>
                                                );
                                            })}
                                            <td className="px-1.5 sm:px-3 py-1 sm:py-1.5 text-center text-[11px] sm:text-[13px] font-black text-indigo-700 dark:text-indigo-400 border-r border-slate-200 dark:border-slate-700 bg-indigo-50/40 dark:bg-indigo-900/40">{formatValue(deptTotalData.total)}</td>
                                            <td className="px-1.5 sm:px-3 py-1 sm:py-1.5 text-center text-[11px] sm:text-[13px] font-black text-rose-500 dark:text-rose-400 bg-rose-50/40 dark:bg-rose-900/40">
                    ...
                    <tfoot className="bg-slate-100 dark:bg-slate-800 font-bold text-[11px] sm:text-[13px] border-t border-slate-200 dark:border-slate-700">
                        <tr>
                            <td colSpan={2} className="px-2 sm:px-3 py-1 sm:py-1.5 text-center text-[10px] sm:text-[12px] font-extrabold text-teal-700 dark:text-teal-300 uppercase tracking-widest sticky left-0 bg-slate-100 dark:bg-slate-800 z-10 border-r border-slate-200 dark:border-slate-700">∑ Tổng</td>
                            {processedData.dateHeaders.map(date => (
                                <td key={date.toISOString()} className="px-1 sm:px-2 py-1 sm:py-1.5 text-center text-[11px] sm:text-[13px] font-extrabold text-slate-800 dark:text-slate-200 tabular-nums border-r border-slate-200 dark:border-slate-700">{formatValue(processedData.totals.daily.get(toLocalISOString(date)) || 0)}</td>
                            ))}
                            <td className="px-1.5 sm:px-3 py-1 sm:py-1.5 text-center text-[11px] sm:text-[13px] font-extrabold text-slate-800 dark:text-slate-200 tabular-nums border-r border-slate-200 dark:border-slate-700">{formatValue(processedData.totals.total)}</td>
                            <td className="px-1.5 sm:px-3 py-1 sm:py-1.5 text-center text-[11px] sm:text-[13px] font-extrabold text-slate-800 dark:text-slate-200">{processedData.totals.daysWithNoSales.toFixed(1)}</td>
                        </tr>
                    </tfoot>
                </table>
            </div>
