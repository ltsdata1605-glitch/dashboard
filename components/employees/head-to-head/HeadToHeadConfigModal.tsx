        ...
        >
            <form onSubmit={handleSave}>
                <div className="p-6 space-y-6 max-h-[70vh] overflow-y-auto">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                         <div>
                            ...
                            <input id="tableName" type="text" value={tableName} onChange={e => setTableName(e.target.value)} placeholder="VD: Thi Đua Sim Số" className="w-full h-11 block rounded-lg border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 pl-3 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm transition-colors" required />
                        </div>
                        <div>
                            <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Chỉ số chính</label>
                            <select value={metricType} onChange={e => setMetricType(e.target.value as any)} className="w-full h-11 block rounded-lg border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 pl-3 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm transition-colors">
                    ...
                    <div className="border-t border-slate-200 dark:border-slate-700 pt-4">
                        ...
                        <div className="flex items-center gap-6">
                            <label className="flex items-center gap-2 cursor-pointer"><input type="radio" value="sum" checked={totalCalculationMethod === 'sum'} onChange={e => setTotalCalculationMethod(e.target.value as any)} className="h-4 w-4 text-primary-600 focus:ring-primary-500" /> Tính Tổng 7 ngày</label>
                            <label className="flex items-center gap-2 cursor-pointer"><input type="radio" value="average" checked={totalCalculationMethod === 'average'} onChange={e => setTotalCalculationMethod(e.target.value as any)} className="h-4 w-4 text-primary-600 focus:ring-primary-500" /> Tính Trung bình 7 ngày</label>
                        </div>
                    </div>
                            ...
                            {conditionalFormats.map(rule => (
                                <div key={rule.id} className="grid grid-cols-1 sm:grid-cols-4 lg:grid-cols-6 gap-3 items-end p-3 bg-slate-100 dark:bg-slate-800/50 rounded-lg animate-fade-in-up">
                                    <div className="sm:col-span-2 lg:col-span-2">
                                        ...
                                        <select value={rule.criteria} onChange={e => updateRule(rule.id, 'criteria', e.target.value)} className="w-full h-9 mt-1 block rounded-md border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm">
                                    ...
                                    <div>
                                        ...
                                        <select value={rule.operator} onChange={e => updateRule(rule.id, 'operator', e.target.value)} className="w-full h-9 mt-1 block rounded-md border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm">
                                            <option value=">">&gt;</option>
                                            <option value="<">&lt;</option>
                                            <option value="=">=</option>
                                        </select>
                                    ...
                                    <div className={`${rule.criteria !== 'specific_value' ? 'hidden' : ''}`}>
                                        ...
                                        <input type="number" value={rule.value} onChange={e => updateRule(rule.id, 'value', Number(e.target.value))} className="w-full h-9 mt-1 block rounded-md border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm" />
                                    </div>
                ...
                <div className="p-4 flex justify-end gap-3 bg-slate-100 dark:bg-slate-800 rounded-b-xl border-t border-slate-200 dark:border-slate-700">
                    ...
                    <button type="submit" className="py-2 px-6 rounded-lg shadow-sm text-sm font-semibold text-white bg-primary-600 hover:bg-primary-700 transition-colors">{editingConfig ? "Lưu" : "Tạo"}</button>
                </div>
            </form>
        </ModalWrapper>
    );
