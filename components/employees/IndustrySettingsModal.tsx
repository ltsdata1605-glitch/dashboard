...
interface SettingsModalProps {
    isOpen: boolean;
    onClose: () => void;
    columns: IndustryColumnConfig[];
    onSave: (newColumns: IndustryColumnConfig[]) => void;
...
const IndustrySettingsModal: React.FC<SettingsModalProps> = ({ isOpen, onClose, columns, onSave, allIndustries, allGroups, allManufacturers }) => {
    ...
    const groupedColumns = useMemo(() => {
        return internalColumns.reduce<Record<string, IndustryColumnConfig[]>>((acc, col) => {
    ...
    const handleMoveGroup = (groupName: string, direction: 'up' | 'down') => {
        ...
        const newColumns: IndustryColumnConfig[] = [];
    ...
    const handleEdit = (column: IndustryColumnConfig) => {
    ...
    const handleSaveColumn = (e: React.FormEvent) => {
        ...
        if (editingColumn) {
            ...
            const newColumn: IndustryColumnConfig = {
                id: `custom_${Date.now()}`,
    ...
    return (
        <ModalWrapper 
            isOpen={isOpen} 
            onClose={onClose} 
            title="Cấu Hình Tiêu Chí Khai Thác" 
            ...
            <div className="flex flex-col h-[85vh] sm:h-auto min-h-0 bg-slate-50 dark:bg-slate-900/50">
                ...
                {view === 'form' && (
                    <ColumnConfigModal
                        ...
                        onSave={(newColumn) => {
                            const mappedColumn: IndustryColumnConfig = {
                                id: newColumn.id,
...
export default IndustrySettingsModal;
