            ...
            <div className="space-y-5">
                <div>
                    ...
                    <div className="inline-flex rounded-md p-1 bg-slate-100/50 dark:bg-slate-900 border border-slate-200/60 dark:border-slate-700 w-full sm:w-auto">
                        <Button type="button" variant="unstyled" size="none" onClick={() => setMetricType('quantity')} className={`flex-1 sm:flex-none py-1.5 px-4 text-xs sm:text-sm rounded transition-colors ${metricType === 'quantity' ? 'bg-white text-slate-800 shadow-sm border border-slate-200 font-medium' : 'text-slate-500 hover:text-slate-800 font-normal border border-transparent'}`}>Số lượng</Button>
                        <Button type="button" variant="unstyled" size="none" onClick={() => setMetricType('revenue')} className={`flex-1 sm:flex-none py-1.5 px-4 text-xs sm:text-sm rounded transition-colors ${metricType === 'revenue' ? 'bg-white text-slate-800 shadow-sm border border-slate-200 font-medium' : 'text-slate-500 hover:text-slate-800 font-normal border border-transparent'}`}>Doanh thu</Button>
                        <Button type="button" variant="unstyled" size="none" onClick={() => setMetricType('revenueQD')} className={`flex-1 sm:flex-none py-1.5 px-4 text-xs sm:text-sm rounded transition-colors ${metricType === 'revenueQD' ? 'bg-white text-slate-800 shadow-sm border border-slate-200 font-medium' : 'text-slate-500 hover:text-slate-800 font-normal border border-transparent'}`}>Doanh thu QĐ</Button>
                    </div>
                </div>
