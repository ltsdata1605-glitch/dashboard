    ...
    return (
        ...
        >
            <SectionHeader
                ...
                icon="layout-grid"
                subtitle={activeTab === 'card' && (
                    <div className="flex items-center">
                        <button onClick={() => handleBreadcrumbClick(0)} className="hover:text-[#0584c7] dark:hover:text-[#0584c7] transition-colors">TẤT CẢ</button>
                        {drilldownPath.map((item, index) => (
                            <div key={index} className="flex items-center">
                                <Icon name="chevron-right" size={3} className="mx-1 opacity-50" />
                                <button onClick={() => handleBreadcrumbClick(index + 1)} className="hover:text-[#0584c7] dark:hover:text-[#0584c7] transition-colors truncate max-w-[120px]">{item.toUpperCase()}</button>
                            </div>
                        ))}
                    </div>
                )}
                    ...
                    <div className="flex bg-slate-100 dark:bg-white/5 p-1 rounded-xl">
                        <button 
                            ...
                            className={`px-4 py-1.5 rounded-lg text-[10px] font-bold uppercase tracking-wider transition-all ${
                                activeTab === 'card' 
                                ? 'bg-white dark:bg-white/10 text-[#0584c7] dark:text-[#0584c7] shadow-sm' 
            ...
            <div className="p-6">
                ...
                currentView.data.length === 0 ? (
                    <div className="flex flex-col items-center justify-center py-16 bg-slate-50/50 dark:bg-white/5 border border-dashed border-slate-200 dark:border-white/10">
                    ...
                    <div className="grid gap-3 grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 industry-cards-grid">
                        {currentView.data.map(({ name, revenue, quantity, icon, color }) => {
                            ...
                            return (
                                <div
                                    ...
                                    className={`group relative flex flex-col justify-between p-3 bg-white dark:bg-[#2c2c2e]/30 border border-slate-100 dark:border-white/5 shadow-[0_2px_8px_rgba(0,0,0,0.02)] hover:shadow-[0_8px_20px_rgba(0,0,0,0.06)] transition-all duration-300 hover:-translate-y-1 ${isDrillable ? 'cursor-pointer active:scale-95' : 'cursor-default'}`}
                                >
                                    <div className="flex justify-between items-start mb-2">
                                        <div className={`w-8 h-8 rounded flex items-center justify-center ${iconClass} transition-transform group-hover:scale-110`}>
                                    ...
                                    <div className="min-w-0">
                                        <div className="text-[9px] font-black uppercase tracking-widest text-slate-400 dark:text-slate-500 truncate mb-0.5" title={name}>{name}</div>
                                        <div className="text-xs font-black text-slate-900 dark:text-white tracking-tight leading-none truncate">{formatCurrency(revenue)}</div>
                                    ...
                                    {isDrillable && (
                                        <div className="absolute bottom-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                            <Icon name="chevron-right" size={2.5} className="text-emerald-500" />
                                        </div>
                                    )}
                                </div>
                            );
                ...
                <div className="flex flex-col lg:flex-row items-stretch gap-6">
                    <div className="lg:w-[45%] p-4 border border-slate-100 dark:border-white/5" ref={pieChartRef} style={{ minHeight: '450px' }}></div>
                    <div className="lg:w-[55%] p-4 border border-slate-100 dark:border-white/5" ref={barChartRef} style={{ minHeight: '450px' }}></div>
                </div>
            )}
            </div>
        </div>
