    ...
    return (
        <div className="w-full overflow-x-auto custom-scrollbar" ref={tableContainerRef}>
            <table className="w-full min-w-[600px] border-collapse bg-white dark:bg-slate-800 rounded-lg overflow-hidden border border-slate-200 dark:border-slate-700 shadow-sm tabular-nums">
                <thead>
                    <tr className="bg-slate-100 dark:bg-slate-700/80 uppercase text-[11px] sm:text-[11px]">
                                ...
                                return (
                                    <tr key={r.id} className={`${trClass} border-b border-slate-200 dark:border-slate-700/50 hover:bg-slate-100 dark:hover:bg-slate-700/50 transition-colors`}>
                                        ...
                                        {config.columns.map(col => {
                                            ...
                                            if (col.type === 'ratio') {
                                                let colorClass = "text-sky-700 dark:text-sky-400 font-black";
                                                
                                                if (col.compareWithTarget && r.targetValue !== undefined) {
                                                    colorClass = val >= r.targetValue 
                                                        ? "text-emerald-700 dark:text-emerald-500 font-black" 
                                                        : "text-rose-700 dark:text-rose-500 font-bold";
                                                }
